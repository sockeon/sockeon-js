...
import type { SockeonMessage, AuthConfig } from './types';
...
export class WebSocketTransport {
  ...
  private buildUrl(): string {
    ...
    if (this.options.auth?.key) {
      url.searchParams.set('key', this.options.auth.key);
    }
   ...
   */
  private isValidMessage(message: any): message is SockeonMessage {
   ...
   * Server will automatically respond with pong (opcode 10)
   */
  sendPing(): void {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      return;
    ...
    try {
      // Send empty ping frame - browser WebSocket API handles opcode 9
      // Note: Browsers don't expose direct ping/pong control,
      // so we'll send a text frame as heartbeat instead
      this.send({ event: '__ping__', data: {} });
      this.log('Ping sent');
    } catch (error) {
